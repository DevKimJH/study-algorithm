package studyAlgorithm.M2207.D4;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < mapArr.length; i++) {
            for (int j = 0; j < mapArr[0].length; j++) {
                if (mapArr[i][j] != -1) sum = sum + mapArr[i][j];
            }
        }
    ...
    static void machineOperate(int[][] mapArr, Node[] nodes) {
        ...
        for (int i = 0; i < 4; i++) {
            while (true) {
                ...
                if (0 <= nextR && nextR <= nodes[0].r && 0 <= nextC && nextC < mapArr[0].length) {
                    if (mapArr[nextR][nextC] == -1) {
                        mapArr[startR][startC] = 0;
                    } else if (mapArr[startR][startC] != -1) {
        ...
        for (int i = 0; i < 4; i++) {
            while (true) {
                ...
                if (nodes[1].r <= nextR && nextR < mapArr.length && 0 <= nextC && nextC < mapArr[0].length) {
